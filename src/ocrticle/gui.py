...
class ImagePreviewScreen(Screen):
    ...
    def show_image(self):
        data = BytesIO()
        i = ImageEnhance.Brightness(self.preview).enhance(
            self.ids.brightness_slider.value / 100
        )
        i = ImageEnhance.Contrast(i).enhance(self.ids.contrast_slider.value / 100)
        ImageEnhance.Color(i).enhance(self.ids.saturation_slider.value / 100).save(
            data, format="PNG"
        )
        data.seek(0)
    ...
    def on_touch_down(self, touch):
        ...
        if min_x <= touch.x <= max_x and min_y <= touch.y <= max_y:
            ...
            new_r = {
                "rect": r_canvas,
                "original_x": (touch.x - min_x)
                * image.width
                / image_p.norm_image_size[0],
                "original_y": (touch.y - min_y)
                * image.height
                / image_p.norm_image_size[1],
            }
            if (
                self.current_mode == "A"
                and len(rectangles) > 0
                and geometry.point_in_rects((touch.x, touch.y), rectangles[-1])
            ):
    ...
    def on_touch_up(self, touch):
        if (
            self.drawing
            and self.current_mode == "A"
            and len(rr := self.rectangles_articles[-1]) > 1
        ):
            new_rect = rr.pop(-1)
    ...
    def on_touch_move(self, touch):
        if self.drawing:
            ...
            width = (
                touch.x
                if min_x <= touch.x <= max_x
                else (min_x if touch.x < min_x else max_x)
            ) - x
            height = (
                touch.y
                if min_y <= touch.y <= max_y
                else (min_y if touch.y < min_y else max_y)
            ) - y

            active_rect["rect"].size = (width, height)
            active_rect["original_width"] = (
                width * self.original_image.width / self.ids.image_p.norm_image_size[0]
            )
            active_rect["original_height"] = (
                height
                * self.original_image.height
                / self.ids.image_p.norm_image_size[1]
            )
        else:
    ...
    def on_resize(self, instance, value):
        for r in self.rectangles_exclude + [
            r for ra in self.rectangles_articles for r in ra
        ]:
            r["rect"].pos = (
                r["original_x"]
                * instance.norm_image_size[0]
                / self.original_image.width
                + (instance.center_x - instance.norm_image_size[0] / 2),
                r["original_y"]
                * instance.norm_image_size[1]
                / self.original_image.height
                + (instance.center_y - instance.norm_image_size[1] / 2),
            )
    ...
    def submit_image(self):
        get_left = (
            lambda r: r["original_x"]
            if r["original_width"] > 0
            else r["original_x"] + r["original_width"]
        )
        get_right = (
            lambda r: r["original_x"]
            if r["original_width"] < 0
        ...
        get_top = (
            lambda r, h: h - r["original_y"]
            if r["original_height"] < 0
            else h - (r["original_y"] + r["original_height"])
        )
        get_bottom = (
            lambda r, h: h - r["original_y"]
            if r["original_height"] > 0
        ...
        if len(self.rectangles_articles) > 0:
            for r in self.rectangles_articles:
                if len(r) == 1:
                    ...
                    top = get_top(r, im.height)
                    bottom = get_bottom(r, im.height)
                    self.manager.article_images.append(
                    ...
                    for rect in r:
                        ...
                        min_top = min(
                            get_top(rect, im.height),
                            min_top or get_top(rect, im.height),
                        )
                        max_bottom = max(
                            get_bottom(rect, im.height),
                            max_bottom or get_bottom(rect, im.height),
                        )
                    ...
                    final_image = Image.composite(im, blank, mask).crop(
    ...
    def save(self, filepath, filename):
        ...
        with open(file, "w", encoding="UTF-8") as f:
            for article in self.manager.articles:
                f.write(
                    article.to_string(self.manager.keep_line_breaks) + "\n\n---\n\n"
                )
